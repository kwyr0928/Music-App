...
const Musicmain = ({ song }: MusicProps) => {
  ...
  const [data, setData] = useState(0); // Arduinoのセンサから取得
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      setData(event.data);
      if (
        (song === "かえるのうた" && tone(event.data) === kaeru[numbers]) ||
  ...
  return (
    <div className="flex flex-col items-center mt-10">
      ...
      <h2 className="mb-4">
        次の音は{" "}
        <span className={`text-5xl ${toneClass2}`}>{kaeru[numbers]}</span>
      </h2>
      <Button
        size="lg"
        colorScheme="primary"
      ...
      <p className="mb-2">
        センサーからの受け取り値{" "}
        <span className={`text-5xl ${toneClass1}`}>{tone(data)}</span>
      </p>
      <p className="mb-2"></p>
      <p>
